<x-layouts.app :title="$title . ' — KN Cosméticos'">
    <div class="kn-container py-6">
        ...
        <!-- Breadcrumbs -->
        <x-store.breadcrumb :items="['Produtos' => '']" />

        <!-- Header Section -->
        <div class="flex flex-col md:flex-row md:items-end justify-between py-6 border-b border-[#E6DED6] gap-4">
            <div>
                <span class="text-xs font-bold uppercase tracking-wider text-[#B0894D]">Vitrine Completa</span>
                <h1 class="font-display text-3xl sm:text-4xl font-semibold text-[#1C1412] mt-1">
                    {{ $title }}
                </h1>
                <p class="text-xs sm:text-sm text-[#6B5F5A] mt-1">
                    Exibindo <span class="font-bold text-[#1C1412]">{{ $totalCount }}</span> {{ $totalCount === 1 ? 'produto encontrado' : 'produtos encontrados' }}
                </p>
            </div>
        ...
        <div class="py-4">
            <x-store.category-pills :categories="$categories" :activeSlug="$filters['category'] ?? null" />
        </div>
        ...
        <div class="flex gap-8 items-start pt-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                @if(count($products) > 0)
                    <x-store.pagination :currentPage="1" :totalPages="2" />
                @endif
            </div>
        </div>
